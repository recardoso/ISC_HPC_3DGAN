...
# train, test fractions
train_fraction = 0.9
test_fraction = 0.1

datasetsize = events_per_file * int(28*train_fraction) # 5000 # 124987
if datasetsize > 124987:
    datasetsize = 124987 # max amount of events
datasetsizetest = 12340
...
def storeResults():
    """
        cp results to 'outputs' so they can be viewed in Azure ML
    """

    print("")
    os.system("echo Content of %s:" % outpath)
    os.system("ls -lah %s" % outpath)
    print("")
    print("")
    os.system("echo Content of %s/results:" % outpath)
    os.system("ls -lah %s/results" % outpath)
    print("")
    print("")
    os.system("echo Content of %s/weights:" % outpath)
    os.system("ls -lah %s/weights" % outpath)
    print("")
    os.system("echo Copying everything inside %s/results to outputs..." % outpath)
    os.system("cp -r %s/results outputs" % outpath)
    os.system("echo Copying everything inside %s/weights to outputs..." % outpath)
    os.system("cp -r %s/weights outputs" % outpath)
    print("")
    #datastore.upload(src_dir='%s' % outpath, target_path='tfresults/runResults', overwrite=True)
...
def get_parser():
    ...
    parser.add_argument('--nbepochs', action='store', type=int, default=nb_epochs, help='Number of epochs to train for.')
    parser.add_argument('--batchsize', action='store', type=int, default=batch_size, help='batch size per update')
    return parser
...
use_autotune = params.autotune
use_cache = params.use_cache
use_prefetch = params.use_prefetch

if use_cache and use_prefetch:
    print("Cache or prefetch but not both")
    sys.exit()

if not params.datapath == '':
    datapath = "%s/tfrecordsprepro/*.tfrecords" % params.datapath
if not params.outpath == '':
    outpath = "%s/tfresults/" % params.outpath
if not params.nbepochs == '':
    nb_epochs = params.nbepochs
if not params.batchsize == '':
    batch_size = params.batchsize

os.system("mkdir -p %s" % outpath + '/weights/3dgan_weights_' + name)
os.system("mkdir -p %s" % outpath + '/results/')

print("Batch Size: %d" % batch_size)
print("Using AUTOTUNE: %s" % use_autotune)
print("Using prefetch: %s" % use_prefetch)
print("Using cache: %s" % use_cache)
...
def RetrieveTFRecordpreprocessing(recorddatapaths, batch_size):
    ...
    if use_prefetch:
        parsed_dataset = recorddata.map(_parse_function, num_parallel_calls=tf.data.experimental.AUTOTUNE).prefetch(buffer_size=tf.data.experimental.AUTOTUNE).repeat().batch(batch_size, drop_remainder=True).with_options(options)
    elif use_cache:
    ...
    # Shuffle
    #parsed_dataset = recorddata.map(_parse_function, num_parallel_calls=tf.data.experimental.AUTOTUNE).shuffle(124987).repeat().batch(batch_size, drop_remainder=True).with_options(options)
...
print("Number of GPUs: %d" % strategy.num_replicas_in_sync)
print("")
